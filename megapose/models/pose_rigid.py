...
class PosePredictor(nn.Module):
    ...
    def render_images_multiview(
        ...
        # # Base directory where the outputs will be stored
        # base_dir = './outputs'
        #
        # # Start from directory outputs/1
        # i = 1
        #
        # # Construct the directory path
        # current_dir = os.path.join(base_dir, str(i))
        #
        # # Loop to find the first non-existing directory
        # while os.path.exists(current_dir):
        #     i += 1
        #     current_dir = os.path.join(base_dir, str(i))
        #
        # # Create the directory once an available spot is found
        # os.makedirs(current_dir)
        #
        # # Loop through each image in the tensor
        # for i in range(render_data.rgbs.shape[0]):
        #     # Get the ith image and remove the channel dimension since it's grayscale
        #     img = render_data.rgbs[i]  # Now the shape should be (240, 320)
        #
        #     img = img.permute(1, 2, 0).contiguous() * 255
        #     # Convert the tensor to a PIL image (need to convert tensor to 8-bit data)
        #     img = Image.fromarray(img.cpu().numpy().astype('uint8'), 'RGB')
        #
        #     # Construct the filename
        #     filename = os.path.join(current_dir, f'image_{i + 1}.png')
        #
        #     # Save the image
        #     img.save(filename)
        #
    ...
    def forward_coarse_with_renders(
        self,
        renders: torch.Tensor,
        images: torch.Tensor,
        K: torch.Tensor,
