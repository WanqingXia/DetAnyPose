...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        # dep_img = cv2.imread(path.replace('images', 'depth'))
        # dep_img = dep_img.transpose((2, 0, 1))
        # img = np.concatenate((dep_img, img), axis=0)
        # img = img[2:, :, :]
...
def load_image(self, i):
    ...
    if im is None:  # not cached in ram
        ...
        if npy and npy.exists():  # load npy
            ...
            dep_path = self.img_files[i].replace('images', 'depth').replace('color', 'depth')
            im = cv2.imread(path)  # BGR
            # dim = cv2.imread(dep_path)
            # im = np.concatenate((im,dim), axis =2)[..., :4]
